...
job_LEFT && _rc=0 || _rc=$?
if [ "$_rc" -eq 0 ]; then _NEEDS_LEFT_RESULT=success; else _NEEDS_LEFT_RESULT=failure; fi

job_RIGHT && _rc=0 || _rc=$?
if [ "$_rc" -eq 0 ]; then _NEEDS_RIGHT_RESULT=success; else _NEEDS_RIGHT_RESULT=failure; fi
