...
# --- Layer 0 ---
job_GREET && _rc=0 || _rc=$?
if [ "$_rc" -eq 0 ]; then _NEEDS_GREET_RESULT=success; else _NEEDS_GREET_RESULT=failure; fi
[ -f "$_TMPDIR/greet.env" ] && source "$_TMPDIR/greet.env" || true
