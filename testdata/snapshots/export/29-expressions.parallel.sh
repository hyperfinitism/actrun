#!/usr/bin/env bash
set -eo pipefail

# Generated from workflow: expressions
# This is an experimental approximation — review before running
# Requires: bash, jq

_TMPDIR=$(mktemp -d)
trap 'rm -rf "$_TMPDIR"' EXIT

# --- GitHub Actions expression helpers ---
gha_contains() { [[ "$1" == *"$2"* ]]; }
gha_startsWith() { [[ "$1" == "$2"* ]]; }
gha_endsWith() { [[ "$1" == *"$2" ]]; }
gha_format() { local fmt="$1"; shift; printf "$fmt" "$@"; }
gha_join() { local IFS="${2:-,}"; echo "${1[*]}"; }
gha_toJSON() { jq -n --argjson v "$(printenv | jq -Rs 'split("\n") | map(select(length > 0) | split("=") | {(.[0]): (.[1:] | join("="))}) | add // {}')" '$v'; }
gha_fromJSON() { echo "$1" | jq -r '.'; }
gha_hashFiles() { cat "$@" 2>/dev/null | sha256sum | cut -d' ' -f1; }
gha_success() { [ "${_NEEDS_LAST_RESULT:-success}" = "success" ]; }
gha_failure() { [ "${_NEEDS_LAST_RESULT:-success}" = "failure" ]; }
gha_always() { true; }
gha_cancelled() { false; }

# ============================================================
# Job: test
# ============================================================
job_TEST() {

  export GREETING=hello

  # --- String functions ---
  echo "contains: $(gha_contains "hello world" world && echo true || echo false)"  # ← echo "contains: ${{ contains('hello world', 'world') }}"
  echo "startsWith: $(gha_startsWith hello hel && echo true || echo false)"  # ← echo "startsWith: ${{ startsWith('hello', 'hel') }}"
  echo "endsWith: $(gha_endsWith hello llo && echo true || echo false)"  # ← echo "endsWith: ${{ endsWith('hello', 'llo') }}"

  # --- JSON functions ---
  echo "toJSON: $(gha_toJSON)"  # ← echo "toJSON: ${{ toJSON(env) }}"

  # --- Env expression ---
  echo "greeting is $GREETING"  # ← echo "greeting is ${{ env.GREETING }}"

}

# ============================================================
# Run jobs
# ============================================================

# --- Layer 0 ---
job_TEST && _rc=0 || _rc=$?
if [ "$_rc" -eq 0 ]; then _NEEDS_TEST_RESULT=success; else _NEEDS_TEST_RESULT=failure; fi
[ -f "$_TMPDIR/test.env" ] && source "$_TMPDIR/test.env" || true

