...
job_PRODUCER() {
  ...
  {
    echo _NEEDS_PRODUCER_OUT_RESULT=$_STEP_COMPUTE_VALUE  # ← ${{ steps.compute.outputs.value }}
  } > "$_TMPDIR/producer.env"
...
[ -f "$_TMPDIR/producer.env" ] && source "$_TMPDIR/producer.env" || true

# --- Layer 1 ---
job_CONSUMER && _rc=0 || _rc=$?
if [ "$_rc" -eq 0 ]; then _NEEDS_CONSUMER_RESULT=success; else _NEEDS_CONSUMER_RESULT=failure; fi
[ -f "$_TMPDIR/consumer.env" ] && source "$_TMPDIR/consumer.env" || true
