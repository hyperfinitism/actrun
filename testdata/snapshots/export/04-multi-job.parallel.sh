...
# Generated from workflow: multi-job
...
job_BUILD() {
  ...
  {
    echo _NEEDS_BUILD_OUT_RESULT=$_STEP_COMPUTE_RESULT  # ← ${{ steps.compute.outputs.result }}
  } > "$_TMPDIR/build.env"
...
job_DEPLOY() {
  ...
  # --- Step 1 ---
  echo "Build result was $_NEEDS_BUILD_OUT_RESULT"  # ← echo "Build result was ${{ needs.build.outputs.result }}"
