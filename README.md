...
- `strategy.matrix` の最小対応 (`axes` / `include` / `exclude` / mixed `axes + include`, `${{ matrix.* }}` の `runs-on` / `run` / `env` / `with`)
