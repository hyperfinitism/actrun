...
- [x] job-level `if:`
- [x] step-level `if:` の `always()` / `failure()` / `cancelled()`
- [x] step-level `continue-on-error`
- [x] `steps.<id>.outcome` / `steps.<id>.conclusion`
- [x] `actions/checkout` の `fetch-depth`
- [x] `actions/checkout` の `ref`
- [x] `actions/checkout` の `clean`
- [x] `actions/checkout` の `submodules`
- [x] `actions/setup-node` の最小 builtin
  - [x] `node-version`
  - [x] `cache`
  - [x] `registry-url`
...
- [x] `${{ vars.* }}` の最小対応
- [x] `${{ secrets.* }}` の最小対応
- [x] `permissions` の parse / contract / reject policy 明確化
- [ ] `actions/cache` の拡張
  - [x] `restore-keys`
  - [x] `lookup-only`
  - [x] `fail-on-cache-miss`
...
- [ ] reusable workflow (`workflow_call`)
  - [x] `on: workflow_call` の parse / contract / reject foundation
- [ ] workflow inputs / outputs
- [ ] job container
- [ ] services
- [x] concurrency の parse / contract / reject policy 明確化
