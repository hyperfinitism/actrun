...
- Added minimal `strategy.matrix` support for axes-only, include-only, exclude filtering, and mixed axes+include workflows, plus docs/E2E coverage for `${{ matrix.* }}` substitution
