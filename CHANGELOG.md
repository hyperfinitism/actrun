...
- Added minimal `strategy.matrix` support for axes-only, include-only, exclude filtering, mixed axes+include workflows, and `max-parallel` throttling, plus docs/E2E coverage for `${{ matrix.* }}` substitution
